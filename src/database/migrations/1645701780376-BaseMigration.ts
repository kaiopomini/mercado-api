...
export class BaseMigration1645701780376 implements MigrationInterface {
    name = 'BaseMigration1645701780376'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`products\` (\`id\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NULL, \`price\` double NOT NULL, \`base_price\` double NOT NULL, \`quantity\` int NOT NULL, \`controlled_inventory\` tinyint NOT NULL DEFAULT 0, \`image\` varchar(255) NULL, \`active\` tinyint NOT NULL DEFAULT 0, \`gtin_code\` varchar(255) NOT NULL, UNIQUE INDEX \`IDX_f28179f6a09c4ad19b3c4d071c\` (\`gtin_code\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`name\` varchar(255) NOT NULL, \`surname\` varchar(255) NOT NULL, \`validated_email\` tinyint NOT NULL DEFAULT 0, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`avatar\` varchar(255) NULL, \`cpf\` varchar(255) NULL, \`gender\` varchar(255) NULL, \`birth_date\` datetime NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`products_products_categories\` (\`product_id\` varchar(255) NOT NULL, \`category_id\` varchar(255) NOT NULL, INDEX \`IDX_4aa4b35c00f6e78a991fcccd71\` (\`product_id\`), INDEX \`IDX_8b220e3a143cdf12103ec047d5\` (\`category_id\`), PRIMARY KEY (\`product_id\`, \`category_id\`)) ENGINE=InnoDB`);
