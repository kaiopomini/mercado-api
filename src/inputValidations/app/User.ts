...
import { validadeCPF, validatePhone, validatePhoneDDI } from "../../utils/validations";
...
export const storeUserSchema = [
  ...
  body("phone")
    .exists({ checkFalsy: true })
    .withMessage("O campo telefone é um campo obrigatório"),
  body("email")
    .isEmail()
    .normalizeEmail()
    .trim()
  ...
  body("password_confirmation")
    .exists({ checkFalsy: true })
    .withMessage("O campo confirmar senha é um campo obrigatório")
    .bail()
    .custom((value, { req }) => value === req.body?.password)
    .withMessage("O campo confirmar senha deve ser igual ao campo senha"),
];
...
export const updateUserSchema = [
  ...
  body("phones.*.isPrimary")
    .optional()
    .isBoolean()
    .withMessage(""),
  body("phones.*.is_whatsapp")
    .optional()
    .isBoolean()
    .withMessage(""),
...
export const updateUserPasswordSchema = [
  body("old_password")
    .exists({ checkFalsy: true })
    .withMessage("O campo senha antiga é obrigatório"),
  body("new_password")
    .isStrongPassword({
      minLength: 8,
      minLowercase: 0,
  ...
  body("password_confirmation")
    ...
    .withMessage("O campo confirmar senha deve ser igual ao campo nova senha"),
];
